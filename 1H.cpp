...
public:
    ...
    int numberOfOccurrences(const string &pattern, int numberOfMismatches) {
        return numberOfOccurrences(0, 0, pattern, numberOfMismatches);
    }
private:
    ...
    int numberOfOccurrences(int state, int position, const string &pattern, int mismatchesLost) {
        if (mismatchesLost < 0) {
            return 0;
        }
        ...
        for (const pair<char, int> &to : states[state].getNextMap()) {
            if (to.first == pattern[position]) {
                ret += numberOfOccurrences(to.second, position + 1, pattern, mismatchesLost);
            } else {
                ret += numberOfOccurrences(to.second, position + 1, pattern, mismatchesLost - 1);
            }
        }
        return ret;
    }
...
int main() {
    ...
    for (int i = 0; i + k <= text.length(); ++i) {
        ...
        dict.insert(getReverseComplement(text.substr(i, k)));
    }
    dict = extendDict(dict, d);

    // Построим суффикный автомат
    ...
    for (const string &s : dict) {
        int currentFrequency = sa.numberOfOccurrences(s, d) + sa.numberOfOccurrences(getReverseComplement(s), d);
        if (currentFrequency > frequency) {
            frequency = currentFrequency;
            mostFrequent.clear();
        }
        if (currentFrequency == frequency) {
            mostFrequent.push_back(s);
        }
    }


    // Вывод результата
    for (const string &s : mostFrequent) {
        cout << s << " ";
    }
    cout << endl;
