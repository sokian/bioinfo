/**
* Решение за O(answer)
* Построим граф де брейна и найдем в нем эйлеров цикл
*/
...
vector<string> genVectorString(int len)  {
    vector<string> ret;
...
int main() {
    ...
    int len;
    cin >> len;
    vector<string> vs = genVectorString(len);
    if (vs.empty()) {
        return 0;
    }
    ...
    // Вывод результата
    // Первую строку
    string eulerCycle = dict[path.front()];
    // Далее только по одному символу
    for (int i = 1; i < (int)path.size(); ++i) {
        eulerCycle += dict[path[i]].back();
    }

    string result = eulerCycle.substr(0, eulerCycle.length() - len + 1);
    cout << result << endl;
